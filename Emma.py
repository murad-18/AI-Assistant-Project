...
def playAssistantSound():
    ...
    music_dir = r'queryBase\Startup2.mp3'
...
def translateText(text):
    ...
    response = requests.post(url, query=payload, headers=headers)
    query = response.json()['query']
    translatedText = query['translations'][0]['translatedText']
...
def get_weather(city: str, query_type: str):
    ...
    r = requests.get(url)
    query = BeautifulSoup(r.text, "html.parser")
    result = query.find("div", class_="BNeawe").text
...
def wishMe():
    ...
    playAssistantSound()  # Play the sound at the beginning

    # Call the wishMe function to greet the user initially
    Speak("Hello Sir")
    wishMe()

    # Start listening after playing the sound
    while True:
        query = takecommand()
        query = str(query).lower()
        if len(query) < 3:
...
if "bye" in query.lower():
            Speak("Goodbye Sir, have a great day!")
            break  # Exit the loop to terminate the program
    if "what is " or "can you tell " or "tell me "or " write ":
        reply = ReplyBrain(query)
        Speak(reply)
